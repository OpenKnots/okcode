...
## 5.4 Vercel marketing/site deployment failures (non-desktop)

Vercel build logs can fail outside the desktop release pipeline with:

- `Error: No Output Directory named ".next" found after the Build completed`
- `You cannot publish over the previously published versions`

### Fixing missing output directory

- Ensure deployment settings use the marketing package context:
  - Framework: `nextjs`
  - Build command: `cd apps/marketing && bun run build`
  - Output directory: `apps/marketing/.next`
- Verify locally:

  ```bash
  bun run build:marketing
  ls apps/marketing/.next
  ```

### Fixing npm publish version reuse

- Do not publish the same package version twice.
- Increase package version before publish and ensure artifacts exist:
...
- If needed, inspect the exact publish output directory referenced by `bin` and repository metadata before release.
