# v0.26.3 Rollout Checklist

Step-by-step playbook for the v0.26.3 release. Each phase must complete before advancing.

## Phase 0: Pre-flight

- [ ] Verify all release package versions are `0.26.3`:
  - `apps/server/package.json`
  - `apps/desktop/package.json`
  - `apps/web/package.json`
  - `apps/mobile/package.json`
  - `packages/contracts/package.json`
- [ ] Verify Android `versionName` and iOS `MARKETING_VERSION` both match `0.26.3`.
- [ ] Confirm `CHANGELOG.md` has `## [0.26.3] - 2026-04-22`.
- [ ] Confirm `docs/releases/v0.26.3.md` exists with Summary, Highlights, Upgrade and install, and Release operations sections.
- [ ] Confirm `docs/releases/v0.26.3/assets.md` exists and lists every expected attachment class.
- [ ] Confirm `docs/releases/v0.26.3/rollout-checklist.md` and `docs/releases/v0.26.3/soak-test-plan.md` exist.
- [ ] Confirm `docs/releases/README.md` includes the v0.26.3 row.
- [ ] Run `bun run release:validate 0.26.3`.
...
- [ ] Create and push tag `v0.26.3`.
- [ ] Verify the coordinated `release.yml` workflow starts.
- [ ] Trigger `release-ios.yml` manually for `v0.26.3` (or the matching release ref if the tag is unavailable).
...
- [ ] GitHub Release body matches `docs/releases/v0.26.3.md`.
...
- [ ] `npm exec --yes --package okcodes@0.26.3 -- okcode --version` returns `0.26.3`.
