# v0.26.6 Rollout Checklist

Step-by-step playbook for the v0.26.6 release. Each phase must complete before advancing.

## Phase 0: Pre-flight

- [ ] Verify all release package versions are `0.26.6`:
  - `apps/server/package.json`
  - `apps/desktop/package.json`
  - `apps/web/package.json`
  - `apps/mobile/package.json`
  - `packages/contracts/package.json`
- [ ] Verify Android `versionName` and iOS `MARKETING_VERSION` both match `0.26.6`.
- [ ] Confirm `CHANGELOG.md` has `## [0.26.6] - 2026-05-11`.
- [ ] Confirm `docs/releases/v0.26.6.md` exists with Summary, Highlights, Upgrade and install, and Release operations sections.
- [ ] Confirm `docs/releases/v0.26.6/assets.md` exists and lists every expected attachment class.
- [ ] Confirm `docs/releases/v0.26.6/rollout-checklist.md` and `docs/releases/v0.26.6/soak-test-plan.md` exist.
- [ ] Confirm `docs/releases/README.md` includes the v0.26.6 row.
- [ ] Run `bun run release:validate 0.26.6`.
...
- [ ] Create and push tag `v0.26.6`.
- [ ] Verify the coordinated `release.yml` workflow starts.
- [ ] Trigger `release-ios.yml` manually for `v0.26.6` (or the matching release ref if the tag is unavailable).
...
- [ ] GitHub Release body matches `docs/releases/v0.26.6.md`.
...
- [ ] `npm exec --yes --package okcodes@0.26.6 -- okcode --version` returns `0.26.6`.
