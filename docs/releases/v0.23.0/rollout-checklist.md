# v0.23.0 Rollout Checklist

Step-by-step playbook for the v0.23.0 release. Each phase must complete before advancing.

## Phase 0: Pre-flight

- [ ] Verify all release package versions are `0.23.0`:
  - `apps/server/package.json`
  - `apps/desktop/package.json`
  - `apps/web/package.json`
  - `apps/mobile/package.json`
  - `packages/contracts/package.json`
- [ ] Verify Android `versionName` and iOS `MARKETING_VERSION` both match `0.23.0`.
- [ ] Confirm `CHANGELOG.md` has `## [0.23.0] - 2026-04-13`.
- [ ] Confirm `docs/releases/v0.23.0.md` exists with Summary, Highlights, Upgrade and install, and Release operations sections.
- [ ] Confirm `docs/releases/v0.23.0/assets.md` exists and lists every expected attachment class.
- [ ] Confirm `docs/releases/v0.23.0/rollout-checklist.md` and `docs/releases/v0.23.0/soak-test-plan.md` exist.
- [ ] Confirm `docs/releases/README.md` includes the v0.23.0 row.
- [ ] Run `bun run release:validate 0.23.0`.
...
- [ ] GitHub Release body matches `docs/releases/v0.23.0.md`.
...
- [ ] `npx --yes okcodes@0.23.0 --version` returns `0.23.0`.
...
- [ ] Monitor reports for regressions in Copilot onboarding, provider auth setup, settings navigation, PR review activity, and desktop update flows.
